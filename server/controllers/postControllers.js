...
const ratingPost = async (req, res, next) => {
    ...
    try {
        ...
        const alreadyRated = post.ratings.find(r => r.userId.toString() === userId);
        ...
        const avg = post.ratings.reduce((acc, curr) => acc + curr.value, 0) / post.ratings.length;
...
const getTotalRatings = async (req, res, next) => {
    ...
    try {
        ...
        const ratingLength = post.ratings.length

        res.status(200).json({ ratingLength })
    } catch (error) {
        console.log(error)
    }
}

module.exports = { createPost, getPosts, getPost, getCatPosts, getUserPosts, editPost, deletePost, ratingPost, getTotalRatings }